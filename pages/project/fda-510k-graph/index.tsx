...
export const DeviceGraph = () => {
    ...
    const searchForNodes = (graphData: GraphData, query: string) => {
        return graphData.nodes.filter((node) => node.id.includes(query) || node.name.includes(query) || node.generic_name?.includes(query))
    }


    const findPredicates = (graphData: GraphData, nodeId: string) => {
        return graphData.links.filter((link: { source: string; target: string }) => link.target === nodeId)
    }

    console.log("selected", selectedNodeData)

    const findSubgraph = (graphData: GraphData, nodeId: string) => {
        const seen = new Set()

        const tempEdges = []
        const tempNodes = []

        const device_tree = findPredicates(graphData, nodeId)

        tempNodes.push(findNode(graphData, nodeId))
        seen.add(nodeId)

        while (device_tree.length) {
            const { target, source } = device_tree.pop()
            if (!seen.has(target)) {
                tempNodes.push(findNode(graphData, target))
                seen.add(target)
            }

            if (!seen.has(source)) {
                tempNodes.push(findNode(graphData, source))
                seen.add(source)

                const sourcePredicates = findPredicates(graphData, source)
    ...
    return <>
        <SearchInput onInputChange={(e) => {
            console.log(e)
            const nodes = searchForNodes(graphData, e)
        ...
        {searchResults.length && <div className={style.SearchResultsWrapper}>
